...
const UsersRoles = () => {
  ...
  const handleClick = clickedUser => {
    let i = index;
    if (clickedUser.email === getCurrentUser().profileObj.email) {
      snackBarContext.openSnackbar('No puedes editar tu rol', 'info');
    } else {
      setUsers(
        users.map(user => {
          if (user.id === clickedUser.id) {
            ...
            const u = { ...user, role: roles[i] };
            return u;
          }
          return user;
        })
      );
      setIndex(index < roles.length - 1 ? index + 1 : 0);
    }
  };
